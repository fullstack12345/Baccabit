...
class Baccarat extends Component {
    ...
    showAllCards(i, thirdCardType='both') {
            var divToMove = document.getElementById('blankCard');
            ...
            var innerDiv = document.getElementById('card-wrapper');
            innerDiv.insertBefore(cln, innerDiv.firstChild);
            var cardType = '';
            var leftPos = '0px'
            const cardSpace = window.innerWidth>767? '15px': '7px'
            const cardWidth = window.innerWidth>767? 96: 48
            
            if(thirdCardType === 'both') {
                if(i === 0){
                    cardType = 'playerfirstCard';
                    leftPos = `calc(25% - ${cardWidth/2}px - ${cardSpace})`
                }
                else if(i===1) {
                    cardType = 'bankerfirstCard';
                    leftPos = `calc(75% - ${cardWidth/2}px - ${cardSpace})`
                }
                else if(i===2) {
                ...
                $(".playerfirstCard")[0].style.left = `calc(25% - ${cardWidth}px - ${cardSpace} - ${cardSpace})`
                $(".playersecondCard")[0].style.left = `25%`
                leftPos = `calc(25% + ${cardWidth}px + ${cardSpace} + ${cardSpace})`
            } else if(thirdCardType === 'banker') {
                cardType = 'bankerthirdCard';
                $(".bankerfirstCard")[0].style.left = `calc(75% - ${cardWidth}px - ${cardSpace} - ${cardSpace})`
                $(".bankersecondCard")[0].style.left = `75%`
                leftPos = `calc(75% + ${cardWidth}px + ${cardSpace} + ${cardSpace})`
            }

            $(cln).addClass(cardType)
            setTimeout(() => {
                cln.style.left = leftPos;
                cln.style.top = '0px';
                cln.style.opacity = 1
            }, 0);
            cardCount ++;
    ...
    openOneByOne(n) {
        setTimeout(function() {
          ...
          $('#card-'+n+'>img')[0].setAttribute('src', 'assets/cards/'+cardObj.f+'.png')
          $('#card-'+n+'>img')[0].setAttribute('id', 'orig-card-'+n)
    ...
    openPlayerThirdCard(i,type, playerDraw, bankerDraw, bankerCardDisplay){
        ...
        // }
        this.createNewCard(n, cardObj, dimensions, playerThirCardValue, type, playerDraw, bankerDraw, bankerCardDisplay);
        this.setState({
    ...
    openBankerThirdCard(i,type, playerDraw, bankerDraw, bankerCardDisplay){
        ...
        // }
        this.createNewCard(n, cardObj, dimensions, bankerThirCardValue, type, playerDraw, bankerDraw, bankerCardDisplay);
        this.setState({
    ...
    createNewCard(n, cardObj, dimensions, thirdCardScore, type, playerDraw, bankerDraw, bankerCardDisplay) {
    ...
    rebet() {
        ...
        const children = Array.from(cardWrapper.children);
        for (let i = 0; i < children.length-1; i++) {
            cardWrapper.removeChild(children[i]);
          }
        // $('#card-wrapper').html('');
        this.reset();
        this.deal();
                    ...
                    <div className="baccarat" style={{ transform: `${window.innerWidth <= 1536 ? `scale(${scale}) translate(${translateX}px, ${translateY}px)` : 'none'}` }}>
                        <div className="baccarat-wrapper">
                            ...
                            <div className="baccarat-card-container">
                                ...
                                <div id="card-wrapper">
                                    <div id="blankCard" className="open-card">
                                        <img src={'assets/cards/hidden.png'} role="presentation" height="144" />
                                    </div>
                                </div>
                            </div>
